...
public class HttpSessionRequestCache extends org.springframework.security.web.savedrequest.HttpSessionRequestCache {
	...
	public SavedRequest getRequest(HttpServletRequest currentRequest,
		...
		if (session != null) {
			return (DefaultSavedRequest) session
	...
	/**
	 * If <code>true</code>, will only use <code>DefaultSavedRequest</code> to
	 * determine the target URL on successful authentication if the request that
	 * caused the authentication request was a GET. Defaults to false.
	 */
	 ...
	 * URL and exception information in a new <code>HttpSession</code> (the
